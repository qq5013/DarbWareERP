...
namespace 邏輯Bll.轉資料
{
    public class 轉傳票Bll
    {
        ...
        public string 轉Accl()
        {
            ...
            DataTable dbf;
            dbf = DbfOdbc資料(@"C:/temps/accl.dbf");
        ...
        public string 轉SliyYear()
        {
            string result = "";
            List<sliyyearm> yearm = new List<sliyyearm>();
            sliyyearm S = new sliyyearm();
            S.linkid = 0;
            yearm.Add(S);
            XmlToSql<sliyyearm>.WriteXml(yearm);
            return result;
        }
        ...
        {
            DataTable dbfm; //主檔
            dbfm = DbfOdbc資料(@"C:/temps/sliym.dbf", "傳票號碼");
            dbfm.Columns.Add("pkid", typeof(int));

            DataTable dbfd; //明細
            dbfd = DbfOdbc資料(@"C:/temps/sliy.dbf", "傳票號碼");
            dbfd.Columns.Add("linkid", typeof(int));
            DataSet ds = new DataSet();
            ds.Tables.Add(dbfm);
            ds.Tables.Add(dbfd);
            DataRelation dr = new DataRelation("傳票號碼", dbfm.Columns["傳票號碼"], dbfd.Columns["傳票號碼"]);
            ds.Relations.Add(dr); //判斷資料有沒有錯誤
            ...
            sliy有傳票號碼欄位賦值(sliy有);
            linkid賦值(sliy有, sliym); //若是賦值不完整，代表主檔缺失
            List<sliy> sliy = new List<Model.sliy>();
            sliy賦值(sliy, sliy有);
            dbfd.Dispose();
            dbfm.Dispose();
            string result = sliy上傳資料庫(sliym, sliy);
            return result;
        }
        private string sliy上傳資料庫(List<sliym> sliym, List<sliy> sliy)
        {
            string 上傳主檔, 上傳明細;
        ...
        private void sliy賦值(List<sliy> sliy, List<sliy有傳票號碼> sliy有)
        {
        ...
        private void linkid賦值(List<sliy有傳票號碼> sliy有, List<sliym> sliym)
        {
        ...
        private void sliy有傳票號碼欄位賦值(List<sliy有傳票號碼> sliy有)
        {
        ...
        private void sliym欄位賦值(List<sliym> sliym)
        {
        ...
        private DataTable DbfOdbc資料(string location, string 排序 = "")
        {
            string locationm = location;
            string sql;
            if (排序 == "")
            {
            ...
            {
                sql = @"SELECT * FROM " + locationm + " order by " + 排序;
            }
            DataTable dbf;
            dbf = dbfodbc.GetOdbcDbfDataTable(locationm, sql);
            亂碼檢查(dbf);
            return dbf;
        }
